...
use ergo_lib::{ergotree_ir::mir::{select_field::SelectField, constant::TryExtractInto}, chain::transaction::UnsignedInput};
...
impl SellBox {
    const CONTRACT_ADDRESS: &'static str = "PLACEHOLDER";
}

impl SpecifiedBox for SellBox {
    fn box_spec() -> BoxSpec {
        let order_owner = RegisterSpec::new(
        Some(SType::SColl(Box::new(SType::SByte))),
        None
        );
        let order_amount = RegisterSpec::new(
            Some(SType::SLong),
            None
        );
...
impl SellProtocol {
    ...
    pub fn action_refund_sell_box(
        ...
        let order_owner = encoding::unwrap_string(
            &sell_box.registers()[0]
        ).unwrap();
    ...
    pub fn get_sell_box_match_spec(
        sell_box: SellBox
    ) -> BoxSpec {
